...
const vmTreeProvider_1 = require("./vmTreeProvider");
const vmScriptUtils_1 = require("./vmScriptUtils");
function activate(context) {
    const scriptUtils = new vmScriptUtils_1.VmScriptUtils();
    const vmTreeProvider = new vmTreeProvider_1.VmTreeProvider(scriptUtils);
    ...
    const commands = [
        ...
        vscode.commands.registerCommand('vmManager.createVm', async () => {
            try {
                ...
                vscode.window.showInformationMessage(`VM '${vmDisplayName}' created successfully!\n` +
        ...
        vscode.commands.registerCommand('vmManager.deleteVm', async (vm) => {
            const confirm = await vscode.window.showWarningMessage(`Are you sure you want to delete VM '${vm.name}'? This action cannot be undone.`, 'Delete', 'Cancel');
            if (confirm === 'Delete') {
                try {
                    await vscode.window.withProgress({
                        location: vscode.ProgressLocation.Notification,
        ...
        vscode.commands.registerCommand('vmManager.copyDetail', async (value, label) => {
            try {
